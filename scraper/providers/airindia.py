...
class AirIndiaScraper(BaseScraper):
    def __init__(self):
        super().__init__(provider_id="AirIndia_Direct")
            ...
            try:
                ...
                for card in flight_cards[:2]:
                    try:
                        ...
                        # Enforce Air India
                        if "air india" not in combined_payload.lower():
                            continue

                        flight_no_match = re.search(r'(AI\s*[-]?\s*\d{3,4})', combined_payload, re.IGNORECASE)
                        if flight_no_match:
                            raw_no = flight_no_match.group(1).upper().replace(" ", "")
                            flight_number = f"AI-{raw_no[2:]}" if "-" not in raw_no else raw_no
                        else:
                            flight_number = "AI-UNKNOWN"
                ...
                print(f"[-] AirIndia scrape failed {src}->{dest}: {e}")
            finally:
                await browser.close()
