...
class IndiGoScraper(BaseScraper):
    def __init__(self):
        super().__init__(provider_id="IndiGo_Direct")
            ...
            try:
                ...
                for card in flight_cards[:2]:
                    try:
                        ...
                        # Enforce IndiGo
                        if "indigo" not in combined_payload.lower():
                            continue

                        flight_no_match = re.search(r'(6E\s*[-]?\s*\d{3,4})', combined_payload, re.IGNORECASE)
                        if flight_no_match:
                            raw_no = flight_no_match.group(1).upper().replace(" ", "")
                            flight_number = f"6E-{raw_no[2:]}" if "-" not in raw_no else raw_no
                        else:
                            flight_number = "6E-UNKNOWN"
                ...
                print(f"[-] IndiGo scrape failed {src}->{dest}: {e}")
            finally:
                await browser.close()
